...
def interoger_snmp():
    ...
    global supervised_results, all_results
    ...
    for equipement in equipment_list:
        ...
        result = {
            ...
            'value': valeur_supervision, 
            'Temps': datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Formatage de la date et de l'heure
        }
...
def demarrer_supervision():
    if not scheduler.get_job('SupervisionJob'):
        scheduler.add_job(id='SupervisionJob', func=interoger_snmp, trigger='interval', seconds=8)
...
def get_supervised_results():
    ...
    # Pas besoin d'ajouter un nouveau timestamp, car chaque résultat a déjà son propre temps
